...
class Convolution_Model(nn.Module):
    ...
    def __init__(self, img_dim, a_dim, use_cuda=True):
        ...
        self.conv = nn.Sequential(  # input is (3,66,200)
            nn.Conv2d(self.channel, 24, kernel_size=5, stride=2, bias=True),  # (3198)
...
class NCP_Model(nn.Module):
    ...
    def count_params(self):
        ...
        num_of_synapses = self.ltc_cell.synapse_count
        # int(np.sum(np.abs(self._sensory_adjacency_matrix)
        num_of_sensory_synapses = self.ltc_cell.sensory_synapse_count
...
class NCPCfC_Model(nn.Module):
    """This class combines ltc_neurons, wiring shape to form NCP."""

    def __init__(self, ltc_cell, conv_head, use_cuda=True):
        """Initialize the object."""
        super(NCPCfC_Model, self).__init__()
        self.loss = nn.MSELoss(reduction='mean')
    ...
    def forward(self, x, timespans=None):
        ...
        for t in range(self.time_sequence):
            ...
            # take the output from the CNN at each time_step
            output, hidden_state = self.ltc_cell(inputs, hidden_state, ts)
            outputs.append(output)
        outputs = torch.stack(outputs, dim=1)
        # the output should have the shape (batch_size, sequence, commands)
        outputs = outputs.view(outputs.shape[0], self.time_sequence, -1)
    ...
    def evaluate_on_single_sequence(self, x, hidden_state=None, timespans=None):
        """Evaluate whole sequence sequentially."""
        # x has the shape (16,channel, width, height)
        y = self.conv_head(x)   # shape of y (1,16,feature number)
        if hidden_state is None:
...
class NCPCfC_Model_smooth(nn.Module):
    """This class combines ltc_neurons, wiring shape to form NCP."""

    def __init__(self, ltc_cell, conv_head, use_cuda=True, smooth_weight=0.1):
        """Initialize the object."""
        super(NCPCfC_Model_smooth, self).__init__()
    ...
    def loss_smooth(self, steering):
        """Calculate smoothness loss to encourage gradual changes in steering angle."""
        loss = 0.0
        for t in range(steering.shape[1] - 1):  # assuming steering shape is (batch_size, time_sequence, 1)
            loss += torch.mean((steering[:, t + 1] - steering[:, t]) ** 2)
        return loss / (steering.shape[1] - 1)
    ...
    def criterion(self, a_imitator, a_exp):
        """Calculate the origin loss."""
        loss1 = self.loss1(a_imitator, a_exp)
        loss2 = self.loss_smooth(a_imitator)
        loss = loss1 + self.smooth_weight *loss2
        return loss, loss1
...
class AttCLF_CfC_Model(nn.Module):
    def __init__(self, ltc_cell, conv_head, use_cuda=True, clf_weight=0.3):
        """Initialize the object."""
        super(AttCLF_CfC_Model, self).__init__()
        self.loss = nn.MSELoss(reduction='mean')
        ...
        self.clf_head = nn.Sequential(
            nn.Linear(self.conv_head.total_features, 64),
            nn.ReLU(),
            nn.Linear(64, 2),  # 输出 k1 和 c1
            nn.ReLU()
        )
    ...
    def forward(self, x, prev_steering=None, timespans=None):
        y = self.conv_head(x)  # CNN特征提取
        hidden_state = torch.zeros((y.shape[0], self.ltc_cell.state_size), device=x.device)
        ...
        for t in range(self.time_sequence):
            ...
            steering = output.squeeze(-1)  # 假设单输出维度为转向角

            if t > 0 and prev_steering is not None:
                delta_steering = steering - prev_steering
                # 计算 CLF 的动态权重
                clf_params = self.clf_head(y[:, t, :])  # Shape: (64, 2)
                k1, c1 = clf_params[:, 0], clf_params[:, 1]
                clf = k1 * (steering ** 2) + c1 * (delta_steering ** 2)
                clf_outputs.append(clf)
            prev_steering = steering
            outputs.append(output)

        outputs = torch.stack(outputs, dim=1)
        clf_outputs = torch.stack(clf_outputs, dim=1) if clf_outputs else None

        return outputs, clf_outputs

    def cal_clf_loss(self, clf_outputs):
        # """
        # Calculate the CLF (Control Lyapunov Function) loss using model outputs and CLF outputs.
        #
        # Args:
        #     outputs (torch.Tensor): Predicted steering values (e.g., shape [batch_size, time_sequence, 1]).
        #     clf_outputs (torch.Tensor): CLF stability parameters (e.g., shape [batch_size, time_sequence, 2]).
        #     true_values (torch.Tensor): True steering values (e.g., shape [batch_size, time_sequence, 1]).
        #
        # Returns:
        #     torch.Tensor: The calculated CLF loss (scalar).
        # """
        # batch_size, time_sequence, _ = outputs.shape
        #
        # # Initialize CLF loss
        # clf_loss = 0.0
        #
        # # Loop through time steps to calculate the CLF terms
        # for t in range(1, time_sequence):  # Start from t=1 to calculate delta
        #     # Calculate steering difference (delta_steering)
        #     # delta_steering = outputs[:, t, :] - outputs[:, t - 1, :]  # Shape: [batch_size, 1]
        #
        #     # Calculate CLF loss for this time step
        #     clf_t =clf_outputs[:, t-1]
        #     # Accumulate loss
        #     clf_loss += clf_t.mean()  # Average over the batch
        #
        # # Normalize CLF loss over time steps
        # clf_loss /= (time_sequence - 1)  # Exclude t=0 as there's no delta
        clf_loss = clf_outputs.mean()

        return clf_loss


    def criterion(self, outputs, clf_outputs, true_values, clf_loss=None):
        """
        ...
        Args:
            ...
            clf_loss (torch.Tensor, optional): CLF loss. If not provided, it will be computed.

        Returns:
            torch.Tensor: Total loss.
        """
        mse_loss = self.loss(outputs, true_values)
        if clf_loss is None:
            clf_loss = self.cal_clf_loss(clf_outputs)
        total_loss = mse_loss + self.clf_weight * clf_loss
        return total_loss, mse_loss

    def apply_constraints(self):
        """
        Apply weight constraints to the base model if it supports them.
        """
        if hasattr(self.base_model.ltc_cell, "apply_weight_constraints"):
            self.base_model.ltc_cell.apply_weight_constraints()

    def evaluate_on_single_sequence(self, x, hidden_state=None, timespans=None):
        """
        Evaluate a single input sequence for steering predictions and CLF values sequentially.

        Args:
            x (torch.Tensor): Input image sequence of shape (time_sequence, channels, height, width).
            hidden_state (torch.Tensor, optional): Initial hidden state of the LTC cell. Default is None.
            timespans (torch.Tensor, optional): Time intervals between frames. Default is None.

        Returns:
            tuple: (outputs, clf_outputs, hidden_state)
                outputs (torch.Tensor): Predicted steering values (time_sequence, 1).
                clf_outputs (torch.Tensor): CLF stability values (time_sequence - 1,).
                hidden_state (torch.Tensor): Final hidden state after processing the sequence.
        """
        # Extract features using the convolutional head
        y = self.conv_head(x.unsqueeze(0))  # Add batch dimension: (1, time_sequence, feature_dim)
        ...
        for t in range(self.time_sequence):
            inputs = y[:, t, :]  # Features for the current time step
            ts = 1.0 if timespans is None else timespans[t].squeeze()

            # Predict steering value
            output, hidden_state = self.ltc_cell(inputs, hidden_state, ts)
            steering = output.squeeze(-1)  # Convert to scalar (steering value)
            outputs.append(output)

            # Calculate CLF value for t > 0
            if t > 0 and prev_steering is not None:
                delta_steering = steering - prev_steering  # Difference in steering
                clf_params = self.clf_head(y[:, t, :])  # Get k1 and c1
                k1, c1 = clf_params[:, 0], clf_params[:, 1]
                clf = k1 * (steering ** 2) + c1 * (delta_steering ** 2)
                clf_outputs.append(clf)
        ...
        # Stack outputs and clf_outputs
        outputs = torch.stack(outputs, dim=1)  # Shape: (time_sequence,)
        clf_outputs = torch.stack(clf_outputs, dim=1) if clf_outputs else None  # Shape: (time_sequence - 1,)

        return outputs, clf_outputs, hidden_state

    def count_params(self):
        conv_params = sum(p.numel() for p in self.conv_head.parameters())
        print(f"Number of parameters in convolutional head: {conv_params}")
        """Count how many params of NCP needs to be trained."""
        num_of_synapses = self.ltc_cell.synapse_count
        # int(np.sum(np.abs(self._sensory_adjacency_matrix)
        num_of_sensory_synapses = self.ltc_cell.sensory_synapse_count
        ...
        ltc_params = 0
        ltc_params += 3 * self.ltc_cell.state_size
        # count cm,gleak,vleak
        # Each synapse has Erev, W, mu and sigma as parameters
        # (i.e 4 in total)
        ltc_params += 4 * (num_of_sensory_synapses + num_of_synapses)
        print(f"Number of parameters in LTC Cell: {ltc_params}")

        clf_params = sum(p.numel() for p in self.clf_head.parameters())
        print(f"Number of parameters in CLF Head: {clf_params}")

        total_parameters = conv_params + ltc_params + clf_params
