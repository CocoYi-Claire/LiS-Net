...
class ConvolutionHead_Nvidia(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, self.img_channel, self.img_height, self.img_width) #(1024,1,66,200) shape (sample_numbers, channels, height, width)
        x = self.conv(x)  # (1024,32,1,18)
        ...
        self.filter_output = list(torch.split(x, 1, dim=1)) #list:32 (1024,1,1,18)

        feature_layer_list = []
        for i in range(self.num_filters):
            # (sample_numbers, height, width)
            self.filter_output[i] = torch.squeeze(
                self.filter_output[i], dim=1) #(1024,1,18)

            # flatten the output of each filter, 1*18 = 18
            self.filter_output[i] = self.filter_output[i].view(-1, 18) #(1024,18)
            # the output of each filter feed into linear layer
            feats = F.relu(self.linear[i](self.filter_output[i])) #(1024,4)
            feature_layer_list.append(feats)

        # concat the features from each filter together
        self.feature_layer = torch.cat(feature_layer_list, 1) #(1024,128)

        feature_layer = self.feature_layer.view(
            batch_size, self.time_sequence, self.total_features) #(64,16,128)
