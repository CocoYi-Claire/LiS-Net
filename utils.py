...
def epoch_policy(dataset, policy, n_epoch, mode, network):
    ...
    for _, (s_o, a_c) in enumerate(dataset):
        ...
        if policy.training:
            ...
            if network == 'NCP' or network == 'NCP-CfC' or network == 'CLF-NCP-CfC':
...
def epoch_policy_smooth(dataset, policy, n_epoch, mode, network):
    """Calculate Training loss."""
    origin_loss_sum = 0.0
    mse_loss_sum = 0.0
    loss_num = 0
    ...
    for _, (s_o, a_c) in enumerate(dataset):
        ...
        origin_loss, mse_loss = policy.criterion(output, a_c)  # original loss
        if policy.training:
            # nn.modules中的attribute, boolean value,
            # indicate if train mode or not. (drop out when in train mode)
            policy.optimizer.zero_grad()
        ...
        origin_loss_sum += origin_loss.data.item()
        mse_loss_sum += mse_loss.data.item()
        loss_num += 1

    origin_loss_sum /= loss_num
    mse_loss_sum /= loss_num
    # loss_num has the information of batches, so how many batches
    print(f"{n_epoch}-th epoch {mode} "
          f"of policy was end: \n\tloss = {origin_loss_sum}")
    return origin_loss_sum, mse_loss_sum

def evaluate_on_single_sequence_smooth(number, dataset, policy, n_epoch, mode):
    """Calculate validation loss."""
    if mode != 'train':
        policy.eval()
    loss_num = 0
    ...
    for i in range(number):
        ...
        for pic, action in dataset.iterate_as_single_sequence(i):
            ...
            action_predicted, rnn_state = policy.evaluate_on_single_sequence(
                pic, rnn_state)
            origin_loss, mse_loss = policy.criterion(action_predicted, action)
    ...
    print(f"{n_epoch}-th epoch {mode} "
          f"of policy was end: \n\t"
          f"Total Loss with smooth = {origin_loss_sum}, "
          f"MSE Loss  = {mse_loss_sum}")
    return origin_loss_sum, mse_loss_sum

def epoch_policy_CLF(dataset, policy, n_epoch, mode, network):
    """Calculate Training loss."""
    origin_loss_sum = 0.0
    mse_loss_sum = 0.0
    loss_num = 0
    ...
    for _, (s_o, a_c) in enumerate(dataset):
        ...
        s_o = s_o.to(policy.device, non_blocking=True)
        a_c = a_c.to(policy.device, non_blocking=True)
        output, clf_outputs = policy(s_o)

        origin_loss, mse_loss = policy.criterion(output,clf_outputs, a_c) # original loss
        if policy.training:
            # nn.modules中的attribute, boolean value,
            # indicate if train mode or not. (drop out when in train mode)
            policy.optimizer.zero_grad()
    ...
    print(f"{n_epoch}-th epoch {mode} "
          f"of policy was end: \n\t"
          f"Total Loss = {origin_loss_sum}, "
          f"MSE Loss  = {mse_loss_sum}")
    return origin_loss_sum, mse_loss_sum

def evaluate_on_single_sequence_CLF(number, dataset, policy, n_epoch, mode):
    """Calculate validation loss."""
    if mode != 'train':
        policy.eval()
    loss_num = 0
    ...
    for i in range(number):
        ...
        for pic, action in dataset.iterate_as_single_sequence(i):
            ...
            # (1,16,channel,width,height)
            action = action.unsqueeze(dim=0)  # from (16,3) to (1,16,3)
            action_predicted, clf_outputs, rnn_state = policy.evaluate_on_single_sequence(
                pic, rnn_state)
            origin_loss, mse_loss = policy.criterion(action_predicted, clf_outputs, action)
